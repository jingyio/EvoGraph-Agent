#!/usr/bin/env python3
"""Run a bounded real-data RSI scale/concurrency showcase without changing taskbank files."""
import asyncio
...
async def main():
    ...
    # Fixed, no-learning concurrency: use the already compiled scale graph and
    # repeat four separate real task scopes. Model and task limits vary together;
    # reads remain serialized to keep this distinct from a tool-concurrency claim.
    fixed = [row['taskId'] for row in rows] + ['finance-cancelled_payments-04']
    bank.tasks['finance-cancelled_payments-04'] = TaskBank().task('finance-cancelled_payments-04') if False else bank.tasks.get('finance-cancelled_payments-04')
    concurrency = []
    for level in [1, 2, 4]:
    ...
    data = dict(id=EXPERIMENT, createdAt=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), protocol=dict(model=config.MODEL, scale='disjoint public train records; 10 cold then 30/60 Fast reuse', concurrency='four frozen task scopes; run/model=1,2,4; reads=1; learning=false'), scale=rows, concurrency=concurrency)
