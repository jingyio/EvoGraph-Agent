...
def render_report(run, task):
    ...
    followup_note = ''
    if isinstance(followup, dict) and followup.get('parentRunId'):
        followup_note = ('<p class="meta">追问关联：基于同一工作区的已保存成果 '
                         + text(followup.get('parentRunId'))
                         + '。父报告仅作交互上下文；本报告的证据仍来自本次工具读取。</p>')
