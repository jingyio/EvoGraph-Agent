...
</style><main><header><small>数字员工 · 业务报告 / 历史记录分析</small><h1>{text(task['title'])}</h1><p class="meta">{text(task['id'])} · {task['recordCount']} 条任务范围记录 · {text(task['asOf'])} · 执行状态 {text(run.get('status', 'unknown'))}</p><p class="status">{quality}</p></header>
