...
def tools_for(request):
    return sandbox_tools(request['scenario']) if request['source'] == 'sandbox' else platform_tools(request['source'])
...
class RunService:
    ...
    def restore(self):
        ...
        for folder in self.directory.iterdir():
            ...
            try:
                ...
                if run['id'] == folder.name and run.get('finishedAt') and isinstance(run.get('events'), list):
    ...
    async def start(self, raw):
        ...
        if request['mode'] == 'fixture':
            provider = FixtureProvider(request['scenario'])
