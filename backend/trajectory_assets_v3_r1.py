...
VERSION='trajectory-review-v3-r1'
...
def _suffix(request, metrics, groups, identifier_field):
    return (request.strip()+ '\n请提交以下整数指标：'+ '、'.join(metrics)
            +'。原因分组必须使用：'+ '、'.join(groups)
            +'；每组提供 reason、condition、count、selectedIds 和当前证据。空组也必须如实提交。'
            +f' 业务ID字段为 {identifier_field}：顶层 selectedIds 和每个分组的 selectedIds 都必须填写该字段的业务值，不能填写工作区 rowId 或 evidenceId；rowId 仅用于 evidenceIds。')
...
def material(spec, records):
    ...
    request=_suffix('请根据本次附件生成内部业务简报，范围只限本次资料。'+template[3]+' 原因可重叠；缺失资料不得当作已完成或零值。给出业务ID、当前证据、汇总指标和内部建议。只生成本地报告，不执行外部操作。',list(metrics),list(groups),_identifier_field(role))
...
def build(root):
    ...
    manifest={'version':VERSION,'taskDesignVersion':'heterogeneous-operations-v3-r1','taskDesign':'48 project-authored heterogeneous business requests over disjoint public records; public business-ID/report-evidence contract clarified; audit labels are offline only','splitPolicy':'train only learning; validation/test frozen and unrun','protocol':{'trainPairs':48,'validationPairs':6,'testPairs':6,'precheckPairs':12,'organization':'three scenarios × sixteen heterogeneous train requests; twelve fixed precheck pairs'},'tasks':[],'sources':{}}
    try:
        loaded={}
...
def install(manager:WorkspaceManager,root,spec):
    directory=Path(root)/'artifacts'/VERSION/spec['id'];workspace=manager.create(spec['scenario'],label=spec['title'],provenance='public_historical_trajectory_v3_r1')
    ...
    internal['deliveryContract']={'requiredTableSlots':list(internal['tableBindings']),'requiredMetricKeys':list(spec['requiredMetricKeys']),'requiredGroupNames':list(spec['requiredGroupNames']),'selectedIdField':_identifier_field(spec['scenario']),'selectedIdPolicy':'selectedIds and groups[].selectedIds use business field values; evidenceIds use observed workspace row IDs','evidenceScope':'current attachment rows; no outside records'}
