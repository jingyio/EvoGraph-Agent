"""Frozen heterogeneous public-record task asset for trajectory RSI V3-r3.
...
VERSION='trajectory-review-v3-r3'
SOURCE_SELECTION_VERSION='trajectory-review-v3'
...
COHORTS = {
    'finance': [
        ('finance-reconciliation', '订单财务复核', 'reconcile', ['多表关联', '一对多', '多原因', '缺失']),
        ('finance-payment-health', '支付结构健康复核', 'payment', ['多笔支付', '状态交叉', '缺失', '分期']),
    ],
    'support': [
        ('support-service-timing', '投诉时效复核', 'timing', ['时间', '多来源', '日期完整性']),
        ('support-response-coverage', '投诉响应资料复核', 'coverage', ['响应资料', '文本', '关联完整性']),
    ],
    'tickets': [
        ('tickets-activity-triage', '技术工单活动分诊', 'activity', ['活动', '指派', '状态']),
        ('tickets-delivery-readiness', '技术工单交接准备度复核', 'readiness', ['排期', '标签', '关联完整性']),
    ],
}


def _finance_thresholds(position):
    delta = 10 if position in {4, 8, 16} else 5
    installments = 12 if position in {6, 8, 14} else 10 if position in {2, 5, 12, 16} else 8
    return delta, installments


def business_request(role, kind, position):
    # Business language only. Machine delivery fields exist solely in the report tool schema.
    opening = {
        'finance': '请基于本次附件完成财务运营复核，范围只限本次提供的资料。',
        'support': '请基于本次附件完成客服运营复核，范围只限本次提供的资料。',
        'tickets': '请基于本次附件完成技术工单运营复核，范围只限本次提供的资料。',
    }[role]
    closings = [
        '请汇总整体情况，列出需要关注的记录、对应原因和资料依据，并给出简短的内部跟进建议。',
        '请形成便于主管查看的内部简报：说明整体情况、待关注事项及其资料依据，并提出简短的后续建议。',
        '请给出本次复核摘要和需要优先处理的清单，说明每项的原因与资料依据，并附简短的内部建议。',
        '请整理成内部复核结论，包含整体情况、需关注事项、原因和资料依据，以及下一步建议。',
    ]
    closing = closings[(position - 1) % len(closings)]
    if role == 'finance' and kind == 'reconcile':
        delta, installments = _finance_thresholds(position)
        body = (
            '重点关注支付金额与订单应收金额明显不一致、分期较高，以及支付或商品资料不完整的订单。'
            f'金额差异严格超过 {delta / 100:.2f} BRL、最大分期达到 {installments} 期及以上时需要重点标记。'
            '缺少支付或商品资料的订单应标记为待核查，不能直接按金额为零认定已完成对账。'
        )
        # A single auditable business-data definition, not an execution plan.
        body += '本次以订单为复核单位；同一订单可对应多笔支付或商品明细，金额原始字段以分保存，报告按 BRL 展示。'
        guard = '仅分析本次附件，不执行退款、改账或其他外部操作。'
    elif role == 'finance':
        _, installments = _finance_thresholds(position)
        body = (
            '请检查订单的支付结构健康情况，重点关注同一订单存在多笔支付、分期达到较高水平、已取消订单仍有支付，'
            '以及支付资料不完整的情形。'
            f'最大分期达到 {installments} 期及以上时需要重点标记；资料缺失应保留为待核查，不能据此推断支付已完成。'
        )
        # Keep the same concise data definition for finance payment-health requests.
        body += '本次以订单为复核单位；同一订单可对应多笔支付或商品明细，金额原始字段以分保存，报告按 BRL 展示。'
        guard = '仅分析本次附件，不执行退款、改账或其他外部操作。'
    elif role == 'support' and kind == 'timing':
        limit = 96 if position == 5 else 72 if position in {4, 6, 9, 13} else 48
        body = (
            '重点关注未及时处理、从受理到转交企业严格超过 '
            f'{limit} 小时，以及日期缺失或前后矛盾的投诉。'
            '日期资料不完整时应列为待核查，不能直接认定已经按时转交。'
        )
        guard = '只生成内部复核结论，不联系消费者或企业，也不修改任何投诉记录。'
    elif role == 'support':
        body = (
            '重点关注公开回复或企业回复资料不完整、消费者叙述需要跟进，'
            '以及投诉与响应资料无法对应的情形。资料缺失应明确保留为待核查，不能据此推断投诉已经解决。'
        )
        guard = '只生成内部复核结论，不联系消费者或企业，也不修改任何投诉记录。'
    elif role == 'tickets' and kind == 'activity':
        threshold = 8 if position in {5, 14} else 5 if position in {3, 6, 9} else 3
        body = (
            '请识别仍处于开放状态且讨论较活跃的事项。'
            f'评论达到 {threshold} 条及以上的开放事项需要重点关注，尤其是尚未指派负责人或缺少活动资料的情况。'
            '不要因为讨论数量直接推断故障严重程度。'
        )
        guard = '只生成内部优先队列和交接建议，不修改工单、指派或里程碑。'
    else:
        body = (
            '请检查开放技术工单的交接准备度，重点关注缺少排期、未指派负责人、带有 bug 标签，'
            '以及活动资料无法对应的事项。原因可以同时存在，资料缺失应作为待核查，不能据此推断工单已经处理。'
        )
        guard = '只生成内部优先队列和交接建议，不修改工单、指派或里程碑。'
    return '\n'.join([opening, body, closing, guard])


def _identifier_field(role):
    return {'finance':'order_id', 'support':'complaint_id', 'tickets':'issue_id'}[role]


def task_plan():
    rows = []
    for role, items in TASKS.items():
        for position, (key, _title, _kind, _request, _groups, _features) in enumerate(items, 1):
            cohort_index = 0 if position <= 8 else 1
            cohort, title, kind, features = COHORTS[role][cohort_index]
            rows.append(dict(id=key, scenario=role, title=title, kind=kind, cohort=cohort,
                             position=position, split='train', precheck=key in PRECHECK[role],
                             auditFeatures=features))
        # Independent records are never learned from or counted as an opportunity.
        for split, positions in [('validation', (2, 12)), ('test', (4, 15))]:
            for ordinal, position in enumerate(positions, 1):
                key, _title, _kind, _request, _groups, _features = items[position - 1]
                cohort_index = 0 if position <= 8 else 1
                cohort, title, kind, features = COHORTS[role][cohort_index]
                rows.append(dict(id=f'{key}-{split[0].upper()}{ordinal}', scenario=role,
                                 title=title + f'（{split}）', kind=kind, cohort=cohort,
                                 position=position, split=split, precheck=False,
                                 auditFeatures=features))
    return rows
...
def _finance(records,kind,position):
    ...
    delta, threshold = _finance_thresholds(position)
    if kind=='reconcile':
...
def material(spec, records):
    role = spec['scenario']
    if role == 'finance':
        tables, metrics, groups = _finance(records, spec['kind'], spec['position'])
    elif role == 'support':
        tables, metrics, groups = _support(records, spec['kind'], spec['position'])
    else:
        tables, metrics, groups = _tickets(records, spec['kind'], spec['position'])
    request = business_request(role, spec['kind'], spec['position'])
    return tables, request, {
        'metrics': metrics,
        'groups': {key: sorted(values) for key, values in groups.items()},
        'selectedIds': sorted({value for values in groups.values() for value in values}),
    }
...
def build(root):
    ...
    manifest={'version':VERSION,'taskDesignVersion':'business-language-cohorts-v3-r3','taskDesign':'48 project-authored business-language requests over disjoint public records; six sequential cohorts of eight requests create real Fast-reuse opportunities; audit labels remain offline only','splitPolicy':'train only learning; validation/test frozen and unrun','protocol':{'trainPairs':48,'validationPairs':6,'testPairs':6,'precheckPairs':12,'organization':'three scenarios × two sequential business cohorts × eight requests; twelve fixed precheck pairs','fastReuseMinimumRate':0.5,'fastReusePotential':{'precheck':'6/12 after six legitimate cold starts','full':'42/48 after six legitimate cold starts'}},'tasks':[],'sources':{}}
    try:
        loaded={}
        ...
        for spec in task_plan():
            role,split=spec['scenario'],spec['split'];pool=sorted([r for r in loaded[role] if sources.partition(r,role)==split],key=lambda r:hashlib.sha256((SOURCE_SELECTION_VERSION+role+split+r['id']).encode()).hexdigest())
...
def install(manager:WorkspaceManager,root,spec):
    directory=Path(root)/'artifacts'/VERSION/spec['id'];workspace=manager.create(spec['scenario'],label=spec['title'],provenance='public_historical_trajectory_v3_r3')
    ...
    internal['deliveryContract']={'requiredTableSlots':list(internal['tableBindings']),'requiredMetricKeys':list(spec['requiredMetricKeys']),'requiredGroupNames':list(spec['requiredGroupNames']),'selectedIdField':_identifier_field(spec['scenario']),'selectedIdPolicy':'Machine report fields use business record IDs for selections and current-workspace row references for evidence.','evidenceScope':'current attachment rows; no outside records'}
