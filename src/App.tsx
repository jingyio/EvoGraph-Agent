...
import { PRESETS, type AgentRun, type DataSource, type PublicConfig, type RunMode, type Scenario, type ToolCard, type World } from '../shared/types';
...
export default function App() {
  ...
  const [page, setPage] = useState<'workbench' | 'tools' | 'platforms'>('workbench');
  ...
  function changeScenario(next: Scenario) {
    setScenario(next); setTask(PRESETS[next]); setRun(null); setRunId(null); setSource('sandbox'); setPage('workbench'); setTab('overview'); setError(''); setTaskCollapsed(false);
  }
  function changeMode(next: RunMode) { setMode(next); setRun(null); setRunId(null); setTaskCollapsed(false); if (next === 'fixture') { setTask(PRESETS[scenario]); setSource('sandbox'); } }
  async function start() {
    setError(''); setSubmitting(true); setRun(null); setRunId(null); setTaskCollapsed(false);
    try {
      const result = await api<{ id: string }>('/api/runs', { method: 'POST', body: JSON.stringify({ scenario, mode, source, task }) });
      setRunId(result.id); setPage('workbench'); setTab('trace');
    } catch (error) { setError((error as Error).message); }
    finally { setSubmitting(false); }
  }
  async function loadHistory(item: History) {
    setScenario(item.request.scenario); setMode(item.request.mode); setSource(item.request.source); setTask(item.request.task); setRun(null); setRunId(item.id); setPage('workbench'); setTab('report'); setError('');
  }
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="history">
        ...
        {history.slice(0, 5).map(item => <button key={item.id} disabled={busy} onClick={() => void loadHistory(item)} className={runId === item.id ? 'selected' : ''}><span className={`dot ${item.status === 'completed' ? 'green' : 'amber'}`} /><span>{names[item.request.scenario]}<small>{new Date(item.startedAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })} · {item.request.mode === 'fixture' ? '离线示例' : '模型运行'}</small></span></button>)}
      </div>
    ...
    <div className="main-shell">
      <header className="topbar"><div><span>工作空间</span><ChevronRight size={14} />{page === 'tools' ? '工具目录' : page === 'platforms' ? '平台连接' : names[scenario]}</div><div className="topbar-right"><span className="environment"><span className="dot green" />本地实验环境</span><span className="avatar">OP</span></div></header>
      <main>
        ...
        {page === 'workbench' && <>
          <div className="page-heading"><div><div className="eyebrow">DIGITAL WORKFORCE / {scenario === 'finance' ? 'FINANCE' : 'CUSTOMER SERVICE'}</div><h1>{names[scenario]}工作台<span className="version">ReAct v0.1</span></h1><p>{scenario === 'finance' ? '从回款核对到异常处理，每一步都有业务依据。' : '从工单巡检到回复准备，让每一项服务动作可追溯。'}</p></div><div className="snapshot-pill"><Database size={15} />{synthetic ? '合成业务快照 · 2026.09.08' : `${source} · 实例只读数据`}</div></div>
          ...
          <section className={`task-card ${taskCollapsed ? 'collapsed' : ''}`}>
            <div className="task-top"><div className="section-title"><span className="icon-tile"><Sparkles size={19} /></span><div><h2>{taskCollapsed ? '本次任务已生成简报' : '交给数字员工'}</h2><p>{mode === 'fixture' ? '固定流程验证工具闭环；本模式不调用 LLM。' : '模型根据任务与工具观察，逐步决定下一项操作。'}</p></div></div><div className="task-top-controls">{run?.report && !busy && <button className="text-button" onClick={() => setTaskCollapsed(!taskCollapsed)}>{taskCollapsed ? '展开任务' : '收起任务'}<ChevronDown size={14} /></button>}{taskCollapsed && <button className="button secondary" disabled={busy} onClick={() => void start()}><RefreshCw size={14} />再次运行</button>}<div className="segmented"><button className={mode === 'fixture' ? 'selected' : ''} disabled={busy} onClick={() => changeMode('fixture')}>离线流程示例</button><button className={mode === 'live' ? 'selected' : ''} disabled={busy} onClick={() => changeMode('live')}>真实模型 ReAct</button></div></div></div>
            <textarea aria-label="任务说明" value={task} readOnly={mode === 'fixture'} disabled={busy} onChange={event => setTask(event.target.value)} />
            <div className="task-bottom"><div className="task-options"><label><Database size={14} /><select aria-label="业务数据源" value={source} disabled={busy || mode === 'fixture'} onChange={event => { setSource(event.target.value as DataSource); setRun(null); setRunId(null); setTask(scenario === 'finance' ? '分页核查可见的应收发票和客户收款，读取必要详情，识别有依据的待核查事项并输出只读运营简报。不要修改账务。' : '读取可见工单及状态和优先级字典，核查工单详情和往来内容，识别服务风险并输出有依据的只读简报。不要发送消息或修改工单。'); }}><option value="sandbox">独立业务沙箱</option><option value={scenario === 'finance' ? 'erpnext' : 'zammad'} disabled={!config?.connectors[scenario === 'finance' ? 'erpnext' : 'zammad']}>{scenario === 'finance' ? 'ERPNext' : 'Zammad'} · 只读</option></select></label><span className="tool-count"><Wrench size={14} />{tools.length} 个工具</span><span className={`mode-note ${mode === 'fixture' ? 'amber-text' : ''}`}>{mode === 'fixture' ? '固定预设 · 无模型指标' : config?.modelConfigured ? config.model : '尚未配置模型'}</span></div>
          ...
          <div className="workspace-grid"><div className="main-panel">
          ...
          </div><aside className="inspector"><section className="panel"><div className="panel-heading"><h2>执行计量</h2><Activity size={16} /></div><div className="runtime-mode"><span className={`dot ${mode === 'live' ? 'green' : 'amber'}`} />{run ? run.request.mode === 'fixture' ? '离线固定流程 · 非 LLM' : run.model : mode === 'fixture' ? '离线固定流程 · 非 LLM' : config?.model || '等待模型配置'}</div><div className="metric-list"><div><span>LLM 请求</span><strong>{run?.metrics.modelRequests ?? '—'}<small>次</small></strong></div><div><span>工具调用</span><strong>{run?.metrics.toolCalls ?? '—'}<small>次</small></strong></div><div><span>输入 / 输出 token</span><strong className="tokens">{run ? `${run.metrics.inputTokens ?? '未知'} / ${run.metrics.outputTokens ?? '未知'}` : '—'}</strong></div><div><span>端到端耗时</span><strong>{run ? (run.metrics.durationMs / 1000).toFixed(2) : '—'}<small>s</small></strong></div></div><div className="meter-caption">{mode === 'fixture' ? '离线示例用于验证流程和界面，耗时不能用于比较模型性能。' : run && !run.metrics.usageComplete ? '部分请求未返回 token 用量；当前 token 统计不完整。' : '记录真实请求与供应商返回用量，不预填成本收益。'}</div></section>
            ...
            <section className="context-card"><div><BookOpen size={17} /><h3>当前实验范围</h3></div><p>基础 ReAct 执行器与业务工具。后续可在相同快照、工具和报告模板上加入经验复用与 motif。</p><span>当前未启用 RSI 优化</span></section>
          </aside></div>
        ...
        {page === 'tools' && <><div className="page-heading"><div><div className="eyebrow">CAPABILITIES / TOOL REGISTRY</div><h1>工具目录</h1><p>{names[scenario]} · {source === 'sandbox' ? '独立业务沙箱' : source} · 参数校验后执行</p></div><span className="snapshot-pill"><Wrench size={15} />{tools.length} 个可用工具</span></div><div className="tool-grid">{tools.map(tool => <section className="panel tool-card" key={tool.name}><div><span className="icon-tile"><Wrench size={18} /></span><span className={`effect ${tool.effect}`}>{tool.effect === 'read' ? '读取 / 计算' : tool.effect === 'artifact' ? '成品输出' : '沙箱写入'}</span></div><h3>{tool.name}</h3><p>{tool.description}</p><details><summary>参数 schema<ChevronDown size={13} /></summary><pre>{JSON.stringify(tool.parameters, null, 2)}</pre></details></section>)}</div></>}
