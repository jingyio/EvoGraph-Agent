...
const pages = { home: '工作台', compare: '验证与对照', live: '现场演示', insights: 'RSI 运行时', replay: '执行记录', experiments: '实验中心', showcase: '已保存展示', demo: '历史执行工作台', evaluation: '成对评测与成果', evolution: '随任务进化', taskbank: '真实数据任务库', platforms: '平台只读工作台' };
type Page = keyof typeof pages;
const primaryPages = ['home', 'compare', 'live', 'insights', 'replay'] as const;
...
export default function App() {
  ...
  if ((primaryPages as readonly Page[]).includes(page)) return <div className="showcase-shell"><header className="showcase-nav"><a href="#home" className="showcase-brand"><Layers3 size={18} /><span>OPERATIONS</span><small>EMPLOYEE</small></a><nav>{primaryPages.map(key => <a key={key} href={`#${key}`} className={page === key ? 'active' : ''}>{pages[key]}</a>)}</nav><a href="#demo" className="nav-lab">实验工作台 <BarChart3 size={14} /></a></header>{page === 'home' && <WorkspaceWorkbench />}{page === 'compare' && <CompareExperience />}{page === 'live' && <LiveComparison />}{page === 'insights' && <RsiInsights />}{page === 'replay' && <TaskReplay />}</div>;
  if (page === 'experiments') return <div className="showcase-shell"><header className="showcase-nav"><a href="#home" className="showcase-brand"><Layers3 size={18} /><span>OPERATIONS</span><small>EMPLOYEE</small></a><nav><a href="#home">工作台</a><a href="#compare">验证与对照</a><a href="#experiments" className="active">实验中心</a></nav><a href="#demo" className="nav-lab">实验工作台 <BarChart3 size={14} /></a></header><WorkpackExperimentPanel /></div>;
