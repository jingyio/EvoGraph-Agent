...
function Aggregate({ summary, baseline }: { summary: Summary; baseline: string }) {
  ...
  return <div className="eval-overflow"><table className="eval-table"><thead><tr><th>指标 · 包含失败成本</th><th>{baseline === 'plan_react' ? 'Plan + ReAct' : 'Strong ReAct'}</th><th>Graph RSI</th></tr></thead><tbody>
...
export default function EvaluationPanel() {
  ...
  return <div className="evaluation-page">
    ...
    {item && summary && <>
      <div className="eval-progress"><strong>{item.protocol.arms[0] === 'plan_react' ? 'Plan + ReAct' : 'Strong ReAct'} vs RSI · {status[item.status]} · {completed} / {item.pairs?.length || 0} 对</strong><progress value={completed} max={item.pairs?.length || 1} /><span>模型 {item.protocol.executor}</span></div>
