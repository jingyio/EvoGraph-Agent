...
type Experiment = { id: string; status: string; split: string; createdAt: string; historicalSetupTokens: number; protocol: { arms: string[]; repeats: number; observedGraphCoverage: number; planner: string; executor: string; sourceHash: string; graphHash: string }; summary: Summary; byScenario: Record<string, Summary>; pairs?: Pair[] };
const fmt = (n?: number | null) => n == null ? '—' : Math.round(n).toLocaleString();
const seconds = (n?: number | null) => n == null ? '—' : (n / 1000).toFixed(1) + ' s';
const armName = (arm: string) => arm === 'strong_react' ? 'Strong ReAct' : arm === 'plan_react' ? 'Plan + ReAct' : arm === 'plan_react_reuse' ? '复用 Plan + ReAct' : 'Graph RSI';
...
function Aggregate({ summary, baseline }: { summary: Summary; baseline: string }) {
  const a = summary.arms[baseline], b = summary.arms.graph_rsi;
  return <div className="eval-overflow"><table className="eval-table"><thead><tr><th>指标 · 包含失败成本</th><th>{armName(baseline)}</th><th>Graph RSI</th></tr></thead><tbody>
...
export default function EvaluationPanel() {
  ...
  const [baseline, setBaseline] = useState('strong_react');
  const [judgements, setJudgements] = useState<{ items: any[]; model: string; rubricHash?: string; sameAsExecutor: boolean }>({ items: [], model: '', sameAsExecutor: false });
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        const votes = key ? await api<typeof judgements>('/api/evaluations/' + key + '/judgements') : null;
        if (!closed) { setItems(list); setItem(detail); if (votes) setJudgements(votes); setLoadError(''); }
  ...
  const busy = pending || items.some(x => x.status === 'running') || judgements.items.some(j => j.status === 'running');
  async function start() {
    setPending(true); setError('');
    try { const r = await api<{ id: string }>('/api/evaluations', { method: 'POST', body: JSON.stringify({ split, repeats, baseline }) }); setSelected(r.id); }
  ...
  return <div className="evaluation-page">
    ...
    {item && summary && <>
      <div className="eval-progress"><strong>{armName(item.protocol.arms[0])} vs RSI · {status[item.status]} · {completed} / {item.pairs?.length || 0} 对</strong><progress value={completed} max={item.pairs?.length || 1} /><span>模型 {item.protocol.executor}</span></div>
      ...
      <Aggregate summary={summary} baseline={item.protocol.arms[0]} />
      <p className="eval-boundary">{summary.note.replace('报告文字未评分。', '上述成功率仅按结构化校验；LLM 文字评分另列。')} 已追溯的历史图来源任务另耗 {fmt(item.historicalSetupTokens)} token，此处不冒充完整研发成本。当前只读分析任务不代表已经完成企业写入流程。</p>
      <details><summary>冻结协议与审计指纹</summary><pre>{JSON.stringify(item.protocol, null, 2)}</pre></details>
      <section className="eval-boundary"><h2>LLM-as-Judge · 匿名双顺序复核</h2><p>裁判 {judgements.model} · {judgements.sameAsExecutor ? '当前与执行模型相同，不视为独立模型裁判' : '与执行模型不同'}。给裁判相同事实证据，不提供策略、耗时或 token；事实、覆盖、可读性各 0–10 分，按 50%/30%/20% 加权为 0–1 reward（质量分，不是成功概率）；交换顺序后取平均，分歧单独标记。</p><p>已结束裁判记录 {judgements.items.filter(j => j.status !== 'running').length} 条 · 涉及 {new Set(judgements.items.map(j => j.pairIndex)).size} 对任务 · 裁判额外请求 {judgements.items.reduce((n, j) => n + j.metrics.modelRequests, 0)} 次 · 裁判 token {judgements.items.reduce((n, j) => n + j.metrics.inputTokens + j.metrics.outputTokens, 0)}（不计入 Agent 执行成本）</p></section>
      <h2>逐任务证据 · 包含未通过结果</h2>
      <div className="eval-pairs">{pairs.map(p => <section key={p.index}><header><strong>{p.taskId}</strong><span>第 {p.repeat} 次 · {p.status}</span></header><div className="eval-pair-arms">{item.protocol.arms.map(arm => { const r = p.runs[arm], id = p.runIds[arm]; return <div key={arm}><b>{armName(arm)}</b>{r ? <><span className={r.evaluation.status === 'passed' ? 'eval-pass' : 'eval-fail'}>{r.status} · {r.evaluation.status}</span><p>{fmt(r.metrics.inputTokens + r.metrics.outputTokens)} token · {r.metrics.modelRequests} LLM · {seconds(r.metrics.durationMs)}</p>{r.evaluation.issues?.length ? <p className="eval-fail">{r.evaluation.issues.join(' · ')}</p> : null}{r.fallback && <p>图回退：{r.fallback}</p>}{r.error && <p className="eval-fail">{r.error}</p>}<a href={`/api/taskbank/runs/${id}/report`} target="_blank" rel="noreferrer">打开业务报告与逐条证据 ↗</a><Review runId={id} /></> : <p>{id ? '执行中，可查看实时轨迹' : '等待调度'}</p>}</div>; })}</div><div className="eval-controls"><button className="button secondary" disabled={busy || p.status !== 'completed' || judgements.items.some(j => j.pairIndex === p.index && j.model === judgements.model && j.rubricHash === judgements.rubricHash)} onClick={async () => { setPending(true); try { await api('/api/evaluations/' + item.id + '/judgements', { method: 'POST', body: JSON.stringify({ pairIndex: p.index }) }); } catch (e) { setError((e as Error).message); } finally { setPending(false); } }}>LLM 裁判评价这对报告</button></div>{judgements.items.filter(j => j.pairIndex === p.index).map(j => <div key={j.id}><p>{j.scoreScale ? '0–10 维度评分 / 0–1 reward' : '旧版 0–4 评分'} · 裁判 {j.status} · {j.result?.winner === 'tie' ? '平局' : j.result?.winner === 'inconclusive' ? '结论不确定' : j.result?.winner === 'graph_rsi' ? 'RSI 评分更高' : j.result?.winner ? '基线评分更高' : '尚无有效结论'} · {j.metrics.modelRequests} 次请求 · {j.metrics.inputTokens + j.metrics.outputTokens} token{!j.metrics.usageComplete && '（统计不完整）'}</p>{j.result?.reports && !j.result.orderConsistent && <p className="eval-fail">交换顺序后结论不一致；保留平均 reward，建议复核。</p>}{j.result?.reports && <div className="eval-pair-arms">{item.protocol.arms.map(arm => { const score = j.result.reports[arm]; if (!score) return null; return <p key={arm}><b>{armName(arm)}</b>事实 {score.dimensions.factuality}/10 · 覆盖 {score.dimensions.coverage}/10 · 可读性 {score.dimensions.readability}/10<br /><strong>Reward {score.reward.toFixed(3)}</strong><br />双顺序范围 {score.rewardRange[0].toFixed(3)}–{score.rewardRange[1].toFixed(3)}</p>; })}</div>}{j.error && <p className="eval-fail">{j.error}</p>}<details><summary>双顺序评分、理由与证据</summary><pre>{JSON.stringify(j.passes, null, 2)}</pre></details></div>)}{p.runIds[item.protocol.arms[0]] && p.runIds.graph_rsi && <a className="eval-replay" href={`/?taskId=${p.taskId}&a=${item.protocol.arms[0]}&b=graph_rsi&left=${p.runIds[item.protocol.arms[0]]}&right=${p.runIds.graph_rsi}#demo`} target="_blank" rel="noreferrer">并排查看这组执行轨迹 →</a>}</section>)}</div>
    </>}
  </div>;
}
