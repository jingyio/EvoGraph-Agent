...
export default function WorkspaceWorkbench() {
  ...
  async function addFiles(files: FileList | File[]) {
    // FileList is live: the input resets immediately after this handler.
    // Snapshot before awaiting initial workspace creation (also protects drop).
  ...
  return <main className="workspace-shell">
    <header className="workspace-topbar"><div className="workspace-brand"><Bot size={19} /><span>上传资料，交给数字员工分析</span></div><div className="workspace-topbar-status"><span><i />本次资料独立保存</span><span title={visibleWorkspace.folderPath}><FolderOpen size={12} />{visibleWorkspace.folderName}</span></div></header>
    <details className="workspace-return"><summary>继续之前的工作</summary><select aria-label="选择已保存工作区" value={workspace?.id || ''} disabled={Boolean(active)} onChange={event => {const id=event.target.value;const old=savedWorkspaces.find(w=>w.id===id);if(!old)return;setRole(old.role);setWorkspace(old);setTableId(old.tables[0]?.id||'');setRun(null);setReadyTask(null);setRequest('');setClarifications([]);setCostConfirmed(false);void refreshWorkspace(id);}}><option value="">选择已保存的业务工作</option>{savedWorkspaces.filter(w=>w.sources.length>0 || w.tasks.length>0).map(w=><option key={w.id} value={w.id}>{w.label} · {w.tasks.length} 次请求</option>)}</select></details>
    <section className="workspace-header"><div><p>选择业务能力 · 输入问题与附件</p><h1>{selectedRole.label}</h1><span>{selectedRole.caption}</span></div><div className="workspace-role-picker">{ROLES.map(item => <button key={item.key} className={item.key === role ? 'selected' : ''} onClick={() => void chooseRole(item.key)} disabled={Boolean(active) || busy === 'workspace'}><small>{item.key === 'finance' ? 'FINANCE' : item.key === 'support' ? 'SUPPORT' : 'ENGINEERING'}</small>{item.label}</button>)}</div></section>
    ...
    <section className="workspace-layout">
      ...
      <section className="workspace-center">
        <div className="workspace-request"><header><div><small>工作要求</small><strong>{readyTask?.followupRunId ? '已准备追问任务' : readyTask ? '已准备当前任务' : '输入业务需求'}</strong></div><span>{active ? run?.phase || '执行中' : readyTask?.followupRunId ? '追问等待启动' : readyTask ? '等待启动' : '未运行'}</span></header>
          ...
          <footer>{!readyTask && <button className="workspace-primary" onClick={() => void prepareTask()} disabled={!request.trim() || Boolean(active) || busy === 'prepare'}>{busy === 'prepare' ? <LoaderCircle size={15} /> : <Sparkles size={15} />}解析请求</button>}{readyTask && <><details className="employee-audit"><summary>执行方式</summary><div className="workspace-strategy-picker" role="group" aria-label="本次工作策略"><button className={strategy === 'graph_rsi' ? 'selected' : ''} onClick={() => setStrategy('graph_rsi')} disabled={Boolean(active)}>经验辅助执行</button><button className={strategy === 'plan_react' ? 'selected' : ''} onClick={() => setStrategy('plan_react')} disabled={Boolean(active)}>模型直接执行</button></div></details><label className="workspace-cost"><input type="checkbox" checked={costConfirmed} onChange={event => setCostConfirmed(event.target.checked)} disabled={Boolean(active)} /><span>我确认启动本次分析并产生模型费用</span></label><button className="workspace-primary" onClick={() => void startWork()} disabled={!costConfirmed || Boolean(active) || busy === 'run'}>{busy === 'run' ? <LoaderCircle size={15} /> : <Play size={15} />}开始工作</button></>}{active && <button className="workspace-stop" onClick={() => void stopWork()}><CircleStop size={15} />取消</button>}</footer>
        </div>
        ...
        {run && <section className="workspace-runtime"><header><div><small>真实执行过程</small><strong>{statusName[run.status] || run.status}</strong></div><div className="workspace-runtime-meta"><span>{run.strategy === 'graph_rsi' ? '经验辅助执行' : '模型直接执行'}</span><span>{run.evolution?.planningPath === 'fast' ? '复用已验证步骤' : run.evolution?.planningPath === 'partial' ? '轨迹局部复用' : run.evolution?.planningPath === 'fallback' ? '根据本次问题规划' : '当前任务结构'}</span></div></header><div className="employee-simple-progress"><span>{active ? "正在分析本次资料" : "本次执行已保存，可查看报告与记录"}</span><span>{run.events.length} 个已记录步骤</span></div><details className="employee-audit"><summary>技术审计 · 查看本次执行过程与参数</summary><div className="workspace-runtime-metrics"><Metric label="LLM 请求" value={formatCount(run.metrics.modelRequests)} hint="真实 provider 请求" /><Metric label="token" value={formatCount(totalTokens(run.metrics))} hint="输入 + 输出" /><Metric label="工具" value={formatCount(run.metrics.toolCalls)} hint="本次执行" /><Metric label="耗时" value={formatMs(run.metrics.durationMs)} hint={run.phase || '端到端'} /></div><GraphView run={run} />{Boolean(run.trajectoryMatch || run.evolution?.trajectoryCompilation) && <details className="workspace-trajectory"><summary>轨迹来源 · G{run.evolution?.generation ?? "—"} / M{run.evolution?.matchVersion ?? "—"} · 当前绑定与真实修订</summary><pre>{JSON.stringify({match: run.trajectoryMatch, compilation: run.evolution?.trajectoryCompilation}, null, 2)}</pre></details>}
          {replayEvents.length > 0 && <><div className="workspace-replay-controls"><button title="上一步" aria-label="上一步" disabled={selectedEventIndex <= 0} onClick={() => setSelectedEventSeq(replayEvents[selectedEventIndex - 1]?.seq ?? null)}><ChevronLeft size={15} /></button><span>步骤 {selectedEventIndex + 1} / {replayEvents.length}</span><button title="下一步" aria-label="下一步" disabled={selectedEventIndex >= replayEvents.length - 1} onClick={() => setSelectedEventSeq(replayEvents[selectedEventIndex + 1]?.seq ?? null)}><ChevronRight size={15} /></button></div><div className="workspace-events">{replayEvents.map(event => <button key={event.seq} className={`${eventChannel(event)} ${selectedEvent?.seq === event.seq ? 'selected' : ''}`} onClick={() => setSelectedEventSeq(event.seq)}><i>{eventChannel(event) === 'model' ? 'M' : eventChannel(event) === 'rsi' ? 'R' : eventChannel(event) === 'tool' ? 'T' : 'C'}</i><span><strong>{eventTitle(event)}</strong><small>{event.elapsedMs == null ? '—' : formatMs(event.elapsedMs)}</small></span>{event.type === 'observation' && event.detail?.ok === false && <em>错误</em>}</button>)}</div>{selectedEvent && <div className={`workspace-event-detail ${eventChannel(selectedEvent)}`}><header><span>{eventChannel(selectedEvent) === 'model' ? 'LLM' : eventChannel(selectedEvent) === 'rsi' ? 'RSI RUNTIME' : eventChannel(selectedEvent) === 'tool' ? 'TOOL' : 'CONTROL'}</span><strong>{eventTitle(selectedEvent)}</strong></header>{eventDetailLines(selectedEvent).length > 0 ? <ul>{eventDetailLines(selectedEvent).map((line, index) => <li key={`${selectedEvent.seq}-${index}`}>{line}</li>)}</ul> : <p>该步骤没有额外可展示字段。</p>}</div>}</>}{run.error && <p className="workspace-run-error">{run.error}</p>}</details></section>}
        {report && <section className="workspace-result"><header><div><FileBarChart2 size={18} /><div><small>交付成果</small><strong>{run?.evaluation.status === 'passed' ? '结构化校验通过' : run?.evaluation.status === 'user_review_required' ? '待用户复核' : '报告未通过校验'}</strong></div></div><div className="workspace-result-actions"><a href={`/api/workspaces/runs/${run?.id}/report`} target="_blank" rel="noreferrer">打开报告 <ArrowRight size={14} /></a><a href={`/api/workspaces/runs/${run?.id}/report/download`} download>下载报告 <ArrowDownToLine size={14} /></a><a href={`/api/workspaces/runs/${run?.id}/selection/download`} download>下载结构化清单 <ArrowDownToLine size={14} /></a></div></header><div className="workspace-result-grid"><div className="workspace-result-metrics">{Object.entries(report.metrics || {}).slice(0, 6).map(([key, value]) => <Metric key={key} label={key} value={typeof value === 'object' ? `${Object.keys(value as object).length} 项` : String(value)} />)}</div><div className="workspace-result-copy"><p>{report.summary || '报告尚未返回。'}</p><span>{report.evidenceIds?.length || 0} 条实际观察证据 · {report.selectedIds?.length || 0} 项清单</span>{report.assumptions?.length ? <small>假设：{report.assumptions.join('；')}</small> : null}</div></div>{report.groups?.length ? <div className="workspace-findings">{report.groups.map(group => <article key={group.name}><header><strong>{group.name}</strong><span>{group.count} 项</span></header><p>{group.reason}</p><small>{group.condition}</small><pre>{group.selectedIds.length ? group.selectedIds.join("\n") : "0 项 · 本次没有满足条件的对象"}</pre><details><summary>本组当前证据</summary><pre>{group.evidenceIds.join("\n")}</pre></details></article>)}</div> : null}{exports.length > 0 && <div className="workspace-downloads">{exports.map(item => <a key={item.path} href={item.path}><ArrowDownToLine size={14} />下载清单 · {item.rowCount} 行</a>)}</div>}</section>}
      </section>
