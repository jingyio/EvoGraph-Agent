...
type WorkspaceTask = { id: string; title: string; task: string; createdAt: string; split: string; scenario: Role; sourceStatus?: string };
...
type Workpack = { id: string; scenario: Role; title: string; split: string; difficulty: string; recordCount: number; task: string; sourceUrl?: string };
...
export default function WorkspaceWorkbench() {
  ...
  async function installWorkpack(pack: Workpack) {
    ...
    try {
      ...
      setRole(pack.scenario); setWorkspace(result.workspace); setReadyTask(result.task); setRequest(result.task.task); setTableId(result.workspace.tables[0]?.id || ''); setRun(null); setRuns([]); setCostConfirmed(false); setClarifications([]);
    } catch (reason) { setError((reason as Error).message); }
    finally { setBusy(''); }
  }
  ...
  async function prepareTask(followupRunId?: string) {
    ...
    try {
      ...
      setReadyTask(response.task || null); setCostConfirmed(false); setFollowup(''); await refreshWorkspace();
    } catch (reason) { setError((reason as Error).message); }
    finally { setBusy(''); }
  }
  async function startWork() {
    ...
    try {
      const result = await api<{ id: string }>(`/api/workspaces/tasks/${readyTask.id}/runs`, { method: 'POST', body: JSON.stringify({ strategy: 'graph_rsi', confirmCost: true }) });
  ...
  return <main className="workspace-shell">
    <header className="workspace-topbar"><div className="workspace-brand"><Bot size={19} /><span>OPERATIONS EMPLOYEE</span><small>WORKSPACE</small></div><div className="workspace-topbar-status"><span><i />本地隔离</span><span>模型思考关闭</span><a href="#compare">已保存对照</a><a href="#experiments">实验中心</a></div></header>
    <section className="workspace-header"><div><p>数字员工工作台</p><h1>{selectedRole.label}</h1><span>{selectedRole.caption}</span></div><div className="workspace-role-picker">{ROLES.map(item => <button key={item.key} className={item.key === role ? 'selected' : ''} onClick={() => void chooseRole(item.key)} disabled={Boolean(active) || busy === 'workspace'}>{item.label}</button>)}</div></section>
    ...
    <section className="workspace-layout">
      <aside className="workspace-sources"><header><div><small>资料</small><strong>{workspace.sources.length} 个文件</strong></div><button onClick={() => input.current?.click()} title="添加资料" disabled={Boolean(active) || busy === 'upload'}><Plus size={16} /></button></header>
        ...
        <div className="workspace-workpacks"><small>可加载工作包 · 可加载后下载原始资料</small>{workpacks.filter(item => item.scenario === role && item.split === 'train').slice(0, 4).map(item => <button key={item.id} onClick={() => void installWorkpack(item)} disabled={Boolean(active) || busy === 'workspace'} title={item.sourceUrl || '公开来源资料'}><span>{item.title}</span><small>{item.difficulty} · {item.recordCount} 条公开来源记录</small></button>)}</div>
      ...
      <section className="workspace-center">
        <div className="workspace-request"><header><div><small>工作要求</small><strong>{readyTask ? '已准备当前任务' : '输入业务需求'}</strong></div><span>{active ? '执行中' : readyTask ? '等待启动' : '未运行'}</span></header>
        ...
        {run && <section className="workspace-runtime"><header><div><small>执行过程</small><strong>{statusName[run.status] || run.status}</strong></div><div className="workspace-runtime-meta"><span>{run.strategy === 'graph_rsi' ? 'Graph RSI' : 'Plan + ReAct'}</span><span>{run.evolution?.planningPath === 'fast' ? 'Fast 复用' : run.evolution?.planningPath === 'fallback' ? 'Fallback' : '当前任务结构'}</span></div></header><div className="workspace-runtime-metrics"><Metric label="LLM 请求" value={formatCount(run.metrics.modelRequests)} /><Metric label="token" value={formatCount(totalTokens(run.metrics))} /><Metric label="工具" value={formatCount(run.metrics.toolCalls)} /><Metric label="耗时" value={formatMs(run.metrics.durationMs)} /></div><GraphView run={run} />
