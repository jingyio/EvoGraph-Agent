  ...
  return <main className="workspace-shell">
    ...
    <section className="workspace-layout">
      ...
      <section className="workspace-center">
        ...
        {run && <section className="workspace-runtime"><header><div><small>真实执行过程</small><strong>{statusName[run.status] || run.status}</strong></div><div className="workspace-runtime-meta"><span>{run.strategy === 'graph_rsi' ? 'Graph RSI' : 'Plan + ReAct'}</span><span>{run.evolution?.planningPath === 'fast' ? 'Fast 复用' : run.evolution?.planningPath === 'partial' ? '轨迹局部复用' : run.evolution?.planningPath === 'fallback' ? 'Fallback' : '当前任务结构'}</span></div></header><div className="workspace-runtime-metrics"><Metric label="LLM 请求" value={formatCount(run.metrics.modelRequests)} hint="真实 provider 请求" /><Metric label="token" value={formatCount(totalTokens(run.metrics))} hint="输入 + 输出" /><Metric label="工具" value={formatCount(run.metrics.toolCalls)} hint="本次执行" /><Metric label="耗时" value={formatMs(run.metrics.durationMs)} hint={run.phase || '端到端'} /></div><GraphView run={run} />{Boolean(run.trajectoryMatch || run.evolution?.trajectoryCompilation) && <details className="workspace-trajectory"><summary>轨迹来源 · G{run.evolution?.generation ?? "—"} / M{run.evolution?.matchVersion ?? "—"} · 当前绑定与真实修订</summary><pre>{JSON.stringify({match: run.trajectoryMatch, compilation: run.evolution?.trajectoryCompilation}, null, 2)}</pre></details>}
