...
import { api } from './api';
import { GraphNode, PairDetail, Showcase, Step, TimelineEvent, duration, number, shorterTool, taskFromHash, total } from './showcase';
import './showcase.css';
...
export default function TaskReplay() {
  ...
  useEffect(() => { api<Showcase>(`/api/showcase/${experimentId}`).then(value => { setData(value); setTaskId(current => value.pairs.some(pair => pair.taskId === current) ? current : 'finance-cancelled_payments-02'); }).catch(error => setError(error.message)); }, []);
  useEffect(() => { if (!taskId) return; setDetail(null); api<PairDetail>(`/api/showcase/${experimentId}/pairs/${encodeURIComponent(taskId)}`).then(setDetail).catch(error => setError(error.message)); }, [taskId]);
