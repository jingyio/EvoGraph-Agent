...
export default function TaskRunPanel({ taskId, defaultStrategy = 'autotool' }: { taskId: string; defaultStrategy?: string }) {
  ...
  return <section style={{ borderTop: '1px solid #ccd4da', marginTop: 18, paddingTop: 12 }}>
    ...
    {detail && <>
      ...
      {detail.strategy === 'motif_first' && <p>AutoTool 惯性：尝试 {detail.metrics.inertiaAttempts ?? 0} · 接受 {detail.metrics.inertiaAccepted ?? 0} · 实际调用 {detail.metrics.inertiaCalls ?? 0} · 拒绝 {detail.metrics.inertiaRejected ?? 0} · 错误 {detail.metrics.inertiaErrors ?? 0} · 本地查询 {detail.metrics.inertiaQueryMs ?? 0} ms · 后续模型请求 {detail.metrics.inertiaRecoveryModelRequests ?? 0}</p>}
      ...
      {detail.motifOnlyComparison && <ComparisonView value={detail.motifOnlyComparison} label="AutoTool 对照：同任务 Motif Only" />}
      ...
      {Object.entries({ '阶段计量': detail.phaseMetrics, 'Plan': detail.plan, 'Composition': detail.compositionPlan, 'AutoTool 惯性尝试与入口判定': detail.toolInertia, '意图与工具召回': detail.retrieval, '本地图选择': detail.graphSelection, '读取 DAG': detail.graph, '报告恢复与评分': { submission: detail.submission, evaluation: detail.evaluation, recovery: detail.reportRecovery } }).map(([label, value]) => <details key={label}><summary>{label}</summary><pre>{JSON.stringify(value, null, 2)}</pre></details>)}
    </>}
  </section>;
}
