...
export default function TaskRunPanel({ taskId, defaultStrategy = 'autotool' }: { taskId: string; defaultStrategy?: string }) {
  ...
  return <section style={{ borderTop: '1px solid #ccd4da', marginTop: 18, paddingTop: 12 }}>
    ...
    {detail && <>
      ...
      {Object.entries({ '阶段计量': detail.phaseMetrics, 'Plan': detail.plan, 'Composition': detail.compositionPlan, 'AutoTool 惯性尝试': detail.toolInertia, '意图与工具召回': detail.retrieval, '本地图选择': detail.graphSelection, '读取 DAG': detail.graph, '报告与评分': { submission: detail.submission, evaluation: detail.evaluation } }).map(([label, value]) => <details key={label}><summary>{label}</summary><pre>{JSON.stringify(value, null, 2)}</pre></details>)}
    </>}
  </section>;
}
