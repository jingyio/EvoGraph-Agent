...
export default function GraphPanel({ graphs, run, busy, onLearn, onUse }: { graphs: TaskGraph[]; run: AgentRun | null; busy: boolean; onLearn: () => Promise<void>; onUse: (graph: TaskGraph) => void }) {
  ...
  return <>
    <div className="page-heading"><div><div className="eyebrow">EXPERIENCE / GRAPH-BASED RSI</div><h1>任务图经验库</h1><p>从真实轨迹提取读取结构，绑定本次数据，按依赖执行。</p></div><button className="button primary" disabled={busy || learning || run?.request.mode !== 'live' || run?.status !== 'completed' || run?.evaluation?.status === 'failed'} onClick={async () => { setLearning(true); try { await onLearn(); } finally { setLearning(false); } }}>{learning ? <LoaderCircle size={16} className="spin" /> : <GitBranch size={16} />}从当前运行学习</button></div>
