...
export type TraceRun = { id: string; taskId: string; strategy: Strategy; status: string; phase: string; createdAt: string; startedAt?: string; finishedAt?: string; traceVersion?: number; models: { planner: string; composition?: string; executor: string }; modelSettings?: unknown; metrics: Metrics; events: TraceEvent[]; evaluation: Evaluation; graph?: { nodes: GraphNode[]; nodeStates: Record<string, string> }; submission?: any; evolution?: { usedVersionId?: string; generation?: number; maintenanceMs?: number; note?: string; planningPath?: string; selectedTinyEdgeIds?: string[] }; error?: string };
