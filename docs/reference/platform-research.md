...
调研日期：2026-09-08；模型验证与真实平台部署状态更新于 2026-09-09。ERPNext、Zammad 已在 v100 部署并完成合成数据 API 验收，详见 [部署验证](../history/platform-deployment-validation-2026-09-09.md)。结论区分官方文档支持、客户端协议验证和真实部署验证，不以仓库存在或 HTTP 首页可达代替业务可用性。
...
2026-09-09 已完成公司/币种/科目初始化、只读 API 用户、6 张发票与 6 笔收款、references 核对、多页列表与写入 403 检查，保存了初始快照。快照恢复演练尚未执行。
...
2026-09-09 已初始化测试 agent、3 个业务组与只读 API token，验证默认组不可见；创建 6 个不同状态和优先级的合成工单，配置 SLA 和业务日历，核验文章接口、分页与实际截止时间。全文搜索未启用，当前按列表和 ID 读取。
...
09-08 仅进行了只读检查，当时受到 Docker 权限限制。09-09 用户完成权限配置后，已验证 Docker 24.0.2 与 Compose v2.18.1 可用，成功启动两个独立 Compose 项目。Docker Hub 网络访问超时，通过可达镜像源按官方摘要下载依赖；未修改全局 Docker 或内核配置。具体实例版本和验证范围见部署说明。

实际采用独立 Compose project、数据卷和回环监听端口 ERPNext 18080、Zammad 18081，通过 SSH 隧道连接本地工作台。未配置对客通知渠道。
...
| 真实 ERPNext / Zammad 部署和数据联调 | 已完成隔离部署、合成数据初始化、分页/详情/权限验收及各一次真实模型只读任务；恢复演练和外部写工具未完成 |
