...
| D37 | 默认工作台只接收用户资料与手写业务请求 | 2026-09-14。`#home` 是实际业务工作区，不展示 Workpack、预置题目、难度或历史 A/B pair。用户文本、原始附件和解析表按 `workspace-id` 落盘保存，运行与追问只关联该工作区，且 `learning_enabled=False`。冻结任务、经验复用链和成对评测只在 `#experiments` 及其按需详情中展示，避免把实验题库伪装成用户工作输入。 |
