...
| 岗位化数字员工展示层 | `#employees`；只读 V4 DTO 和保存 HTML 报告 | 财务运营助手、客服运营分析师、技术研发管理助手各自展示一个保存任务、当前参数绑定、实际工具结构、提交指标与领域内 12 项成本对比 | 这是产品/录制层，不是新实验：没有模型调用、没有新学习、没有新报告评分。报告为可打印 HTML，不是 PPTX；岗位映射不能被表述为生产企业系统部署。详见 [岗位展示设计](enterprise-operations-employee-showcase-2026-09-11.md)。 |
