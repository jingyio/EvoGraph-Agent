...
当前持久化图只有三份 G0：tickets/unassigned 为 `family-supported`，support/responses、finance/payment_totals 为 `probation`。这些只是适用证据状态，不是三代图。切换执行环境后需重新读取 `artifacts/online-graphs.json` 核验，不能依赖本页旧快照。
...
1. 历史图/Plan 缓存各自带来的独立收益；现有架构对照还包含 Plan、工具检索、图执行等因素。
...
- 图可能过度遍历。例如取消订单核查先筛选再读取可比全量 foreach 更省工具；需要训练证据支持的按需读取。
...
清理后：61 个 Python 测试、4 个前端回放测试、前端构建、300 条任务库契约校验通过。测试数量变化来自删除旧沙箱专用功能及其测试；当前在线进化、Judge、成对评测与通用协议测试保留。
