...
## 2026-09-14 V3 9B 预检失败与 V3-r1 候选

`qwen/qwen3.5-9b` 的 V3 预检 `350cd048-9b15-4ead-bae5-073b03ca63ad` 在首个财务 F01
按 `quality_stopped` 终止。Baseline：6 次模型请求、10 次工具调用、51,198 token、180.005
秒、usage 不完整，未提交报告。RSI：5 次模型请求、4 次工具调用、60,202 token、175.136
秒、usage 完整，提交的指标正确但 `selectedIds/groups` 使用了工作区 `rowId` 而非业务
`order_id`。两臂均未通过质量门槛；表面 token 差异不构成节省结论。该工件已作为历史发布
完整保留，且不能与此前 27B 中断诊断或任何历史实验混合。

当前候选是 `trajectory-p05-v3-r1-candidate` / `trajectory-review-v3-r1`，尚未开始。它使用
相同的 60 项公开记录和任务顺序，只增加公开业务 ID / 证据 ID 交付边界，并在执行阶段要求
工具调用以抑制无调用长文本。必须从新的空 RSI 经验库重新进行 12 对严格预检；只有全部
通过质量和 usage 门槛后才允许启动 48 对正式对照。下面较早的“not-started”记录仅描述当时的
V3 状态，不代表当前 V3-r1。


2026-09-14 正式运行前审阅已加入当前证据页：六个业务契约、输入字段、48个train题面及
冻结hash均可展开核对，hash漂移会失败关闭。当前仍为`not-started`，等待用户批准后仅启动
9对严格串行预检。自然cohort中8/48任务为空清单，若干缺失类分组恒为空；它们保留为
负向控制，不伪造正例。六类任务的报告工具已前置强制精确metrics键、groups必填与合法
组名；两臂仍共享同一底层工具面。限制见[运行审批审阅](trajectory-v3-run-approval-review-2026-09-14.md)
和[任务与工具审阅](trajectory-v3-task-and-tool-review-2026-09-14.md)。本轮验证为69项相关Python、
7项前端、TypeScript/Vite构建和diff检查；模型调用仍为0。

2026-09-14 任务资产统一完成，但没有新增模型实验。当前Release Manifest改为
`trajectory-p05-v3-candidate` / `trajectory-review-v2`，固定48 train、6 validation、6 test，
六个业务问题契约同时生成根目录 `test/` 的手工问题。9对严格预检和48项full均未启动，
所以当前证据显示计划48、已运行0、无token/latency曲线、无质量或G/M结论。旧V2失败预检
改为historical并保留全部4对运行、失败和开销。前端已恢复同release保存工件的单任务与
累计token/串行latency节省率曲线；只有新候选产生运行后才会出现点。详情见
[V3资产与曲线记录](trajectory-v3-asset-and-curves-2026-09-14.md)。

2026-09-14 前端版本隔离完成时：`releases/manifest.json` 曾固定候选
`trajectory-p05-v2-candidate` / `96c69be0-fc62-494a-b05d-2267d0925c03`。当前证据仅展示
该工件的4对已启动任务、未过门槛与未有修订后使用；没有新的formal发布。旧V17/V4
及其他预检进入开发与历史，不加入当前卡片或曲线。本轮0模型/0Judge调用，原始实验
结论与工件均未改写；38份实验/经验文件只读检查前后hash一致。
验证：7项前端测试、39项相关Python测试、构建和diff检查通过；1280/390px浏览器闭环通过。
详见 [前端版本隔离验收](frontend-release-isolation-2026-09-14.md)。以下数字为历史索引，
不得将不同条目组合为当前发布成绩。

2026-09-14 P0.5执行新增两次预检，整体未过质量门槛。V1 `0795dece` 首对Baseline
分组证据失败；V2 `96c69be0` 财务三对通过，客服首对Baseline误交内部rowId，最终
3/4 vs4/4，token270092→124617仅作诊断。技术付费预检及full未运行。
财务3对结构化门槛通过的子集为165949→95996token（-42.1533%）；全文未独立评审。
实际财务G0→G1与覆盖M1已保存，但后续使用0，进化验收未完成。两版本全开销
614150token/54模型请求，不能跨版本拼接收益。详细结果、失败、限制与录制路径见
[轨迹V2结果](trajectory-v2-results-2026-09-14.md)。新工具/上传/前端实现与历史V17分开。

2026-09-14 新重构交接（TODO P0.5）：目标为实际轨迹诱导图、编译时生成并由正常train
反馈修订匹配描述，以及读取/计算/本地产物复用。首要验收为进化、效率、可展示性，
效果与可靠性作为质量门槛。详见[执行规格](trajectory-rsi-refactor-handoff-2026-09-14.md)。
上述目标随后已进入执行，结果见本页顶部与V2记录；以下历史数字保持原义。

2026-09-14 手工输入材料随后更新：`test/` 的六个问题与 `trajectory-review-v2` 共用公开
问题契约；Excel仍使用原先选取的真实公开记录，补充问题实际需要的源字段投影和明确派生列。
六份附件再次通过逐值上传解析和任务创建检查（0模型请求）。这不是在线对照、Agent成功率
测试或新增学习证据；原实验结论不变。
