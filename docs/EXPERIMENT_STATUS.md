...
| 最终展示与严格摘要审计 | 只读 `online-rsi-serial-final-v4` artifact；`/api/showcase/...` | 展示结构化精确通过 Baseline/RSI 均 36/36；额外摘要 audit 为 Baseline 25/36、RSI 26/36，主要保留未明确金额单位和无法由有限规则确认的数字主张 | 新 audit 没有重跑 Agent/Judge、没有改写历史结果或泄漏 gold。它提高结果展示的可审计粒度，但不等于完整自然语言事实评分；最终文字质量仍受同模型 Judge 覆盖 26/36 和顺序分歧限制。 |
