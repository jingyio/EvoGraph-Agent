...
## 2026-09-14 V3 9B 预检失败、V3-r1 历史候选与 V3-r2 当前候选
...
V3-r1 现为 historical/not-started：它修复了业务 ID 与证据行 ID 的公开契约，但用户题面仍
混入机器交付字段，因此没有启动。它不产生任何质量、效率、可靠性、Fast 或进化结论。

当前候选为 `trajectory-p05-v3-r2-candidate` / `trajectory-review-v3-r2`，尚未开始。它复用
同一批公开记录选择和私有评分边界，但重新冻结了 48 条自然业务题面。题面仅含业务目标、
阈值/口径、缺失语义、业务成果和禁止动作；动态工具 Schema 保存机器可检查的报告格式，
该格式不进入模型消息或图匹配输入。48 个 train 为六个连续 cohort，每组 8 项；实际 RSI
Fast 命中率必须达到 50% 才能通过当前候选发布门槛。没有任何付费模型调用发生在这次重构中。


2026-09-14 V3-r2 的正式运行前审阅已更新到当前证据页：六个连续业务 cohort、输入字段、48 个
自然题面与冻结 hash 均可展开核对，hash 漂移会失败关闭。当前仍为 `not-started`，等待用户
批准后才启动 12 对严格串行预检。报告工具的机器字段仍严格校验，但不出现在用户题面、模型
初始消息或轨迹匹配输入。预检/正式运行的 Fast 实际命中率需要 `>=50%`；未达标不会补跑或
改口径。本轮模型调用为 0。

此前 `trajectory-review-v2` / 9 对预检的描述属于历史资产与历史候选；V2 失败工件、失败和
开销完整保留在历史区，不进入 V3-r2 当前发布，也不与其曲线或结论拼接。
