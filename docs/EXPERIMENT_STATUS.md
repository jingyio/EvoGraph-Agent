## 2026-09-15 · 仓库目录整理（非实验）

文档和冻结 benchmark 已按当前入口、参考资料与历史记录分层。历史协议、失败诊断和旧资产仍在仓库中并保持原内容；Release Manifest、Analysis Manifest、实验 artifacts、指标与结论均未改变。本轮没有模型调用。

## 2026-09-15 · 槽绑定回退修复与 RSI-only 真实诊断（最新）

- 失败诊断来源：严格串行 comparison `a2aabd3d-6f96-414a-ac74-e7ecf8948cc7`。其 C 臂 `49fbaad7-30fb-4afb-a0b8-104c06fdca89` 的 matcher 排除 `t13/t17`，但提交 `t13_threshold/t17_threshold`，因旧槽集合全等校验回退冷规划；14请求、192,081总token、78.650秒。
- 最小修复只规范化未选闭包的多余槽；缺失、重复、题面来源、类型和单位仍严格校验。历史失败工件未改写。
- 用户要求不再完成新的串行三臂验证。已取消新的不学习臂，保留其取消前10次请求和成本；传统臂已经完成，同样保留但不用于本次结论。
- RSI-only run `1c904b6b-9f90-46f6-b3f8-b0a522ab6998`：`qwen/qwen3.5-27b`、thinking关闭、3请求、50,787总token、25工具、0工具错误、33.108秒、usage完整；实际使用 G2/M2 `1fd5c91d-64f4-4e7f-8193-a9b41d64b158` 的23节点，随后模型补一个未覆盖比较并发布报告。
- 结果为16订单、支付与商品加运费均496,883分、差异0、10期及以上3个且ID一致、缺失0；`user_review_required`、issues为空。它验证路径恢复，不是正式A/B对照，不进入Release Manifest或当前正式指标。
- 验证：Python 264项、前端39项、生产构建与diff检查通过。

## 2026-09-15 · 正常 Agent 路径恢复的并行诊断（历史）

- 历史审计确认正常FX12与当前版本使用相同基础system prompt；输出退化主要来自长evidence ID复制、冻结经验未召回和演示专用首份报告终止。历史坏run `cd632ca5…` / `e7a558a5…` 保持不动。
- 后端已恢复共享细粒度工具、当前附件证据范围、运行时证据绑定、共享报告恢复和受控冻结版本兼容召回。三臂使用同题、同附件、同27B模型、同预算；A/B主Key、C Secondary Key并行。
- 唯一新真实API comparison：`9c354b04-6373-4333-80d2-8967b8d50dc1`。A传统Plan+ReAct为15请求、185,957总token、128.278秒；B图执行不学习为12请求、175,000总token、95.890秒；C在线RSI为3请求、52,011总token、31.441秒。三臂业务结果一致：16订单、支付与商品加运费均496,883分、差异0、高分期3、缺失0。
- C实际使用冻结版本`1fd5c91d-64f4-4e7f-8193-a9b41d64b158`（G2/M2），23个节点由图执行，10期阈值由当前题面重新绑定，再由模型补1个未覆盖计算。报告请求输出为602 token；A/B为791/808 token。
- 本次是自由工作区演示验证，三臂状态均为`user_review_required`，不含私有gold，不替换当前金融12任务candidate，也不进入Release Manifest指标。未循环重跑，未删除失败或成本。
- Python264项、前端39项、生产构建与diff检查通过。

## 2026-09-15 · 严格串行三臂与工作区恢复修复（最新）

- 新三臂实测已从并行改为同一主 Key 的 A→B→C 严格串行，工作区调度峰值限制为 `run/model/read=1/1/1`。新记录保存 `strict_serial_three_arm`；旧并行 comparison 保持历史策略。
- comparison `e68639ea-b793-4503-9f30-fa429d01a3ad` 中 A/B 的受限停止已定位为终态报告时间未预留和重复确定性计算循环，不是服务端限流；旧失败、token、请求和时长完整保留。
- 执行器新增共享的报告预算预留和重复计算终止边界。刚结束的严格串行 comparison `b9cd8b3f-c990-475a-961b-978e57d287fe` 显示：A 在180秒总时限内未提交报告；B 在首次报告因一条抄写错误的 evidence ID 校验失败后，又等待约82秒恢复请求才超时；C 用73.381秒完成。新演示策略在完整对账后直接生成精简报告，并在首次报告保存后立即结束当前臂。失效附件的历史任务在页面自动恢复时静默清除本地引用，后端审计记录不删除。
- Python 262项、前端39项、生产构建、diff检查和浏览器检查通过。本轮没有运行新的付费模型实验，当前Release/Analysis Manifest未变。

## 2026-09-15 · 三臂实测发布对齐与页面精简（最新）

- 传统 `Plan + ReAct`、图执行不学习、在线 RSI 三臂实现完成；三个runner与模型槽可同时进入首个模型请求，本地读取槽不再让C臂排队。
- 在线 RSI 经验从数据分析默认 Release Manifest 动态解析并完整校验，当前仍指向金融12任务candidate；演示任务只读，不回写知识库。
- 实测首屏已精简为三种方法、A→B/B→C归因和完整题面；协议、Key和知识库身份默认折叠，真实阶段与运行结果仅在启动后出现。
- Python 259项、前端38项、生产构建与diff检查通过；1280px浏览器无横向溢出。费用确认未勾选，没有新增付费模型run。

## 2026-09-15 · 实测对比三臂与真实阶段显示

- 本轮没有启动新的付费模型运行，也没有修改Release/Analysis Manifest或历史实验工件。
- 新实测将传统 `Plan + ReAct` 加回为第三个真实参考；另两臂仍为同一 `graph_rsi` 的空经验不学习与金融12任务冻结知识库只读RSI。A→B展示运行时差异，B→C隔离学习贡献。
- 工作区演示调度从`2/2/1`改为`3/3/3`，避免三臂创建后RSI在共享本地读取槽排队。主Key承载传统与不学习臂，Secondary Key承载RSI臂；正式实验的冻结协议未改变。
- 前端三栏、三臂瞬时通知和真实阶段对照已接入。阶段持续时间来自保存事件时间戳，不显示模拟百分比。旧双臂与旧Plan/Graph历史记录仍可恢复。

## 2026-09-15 · 实测对比协议与当前证据对齐

- 新实测尚未产生付费run；本轮只修改执行协议和展示。两臂均为27B `graph_rsi`：A空经验不读写，B只读加载金融12任务实验 `d02f0ecd-8bb5-4359-94be-e7f7233df6a5` 已形成的3个版本。
- 旧双Key冒烟 `c762fcdc-e08e-422b-80b0-5ea6968968e6` 仍是历史 `plan_react` vs 冷启动 `graph_rsi`，其2,024 token差和当前数据分析的55.8%观察不可比较，也不会被新协议改写。
- 当前Release/Analysis Manifest仍只绑定原金融12任务candidate。实测页读取该知识库不会写回，数据分析页仍只读原实验工件。

## 2026-09-15 · 数据分析展示收口（未运行模型）

- 当前 Release/Analysis Manifest 仍绑定金融12任务 candidate `d02f0ecd-8bb5-4359-94be-e7f7233df6a5`；实验工件、指标算法和历史结果均未修改。
- `#analysis` 首屏重排为业务质量、token/模型请求效率、G/M修订及后续使用三张主卡；美元成本、串行时长和实际图执行保留为次级计量。
- 五个标签继续共享同一保存结果回放。成本页三张累计曲线并列；报告页移除截断摘要、冻结子簇、逐任务账本、发布后维护诊断、数据限制长列表和模型请求内部分类，只保留完整题面、两臂完整报告、清单、真实轨迹、工具调用与真实G/M diff。FX11失败及全部恢复开销仍保存在原工件和任务深链中。
- 浏览器已检查桌面五标签、报告双栏与折叠审计；390px下页面 `scrollWidth=innerWidth=390`。前端35项、生产构建和`git diff --check`通过。

## 2026-09-15 · 工单独立12项实验准备（最新）

- 财务不重复运行；最终财务上下文继续绑定候选实验 `d02f0ecd-8bb5-4359-94be-e7f7233df6a5`。
- 技术工单新模式固定 `tickets_probe=T01,T02`、`tickets_formal=T01–T12`，使用双 Key 按 pair 并行；27B统一规划/执行/组合，thinking关闭。
- 工单预检和正式实验均从独立空经验开始；不学习臂禁止读写跨任务经验。正式实验须由同 runtime、同资产且两臂全通过、usage完整、维护正常、真实图执行率至少50%的预检解锁。
- 客服和旧混合预检不删除、不改写，只在历史区审计；财务与工单结果将作为两个独立场景展示，不生成跨实验合计收益。
- 当前尚未生成新的工单实验 ID；完成协议回归并提交冻结 runtime 后立即运行真实 `tickets_probe`。

## 2026-09-15 · 财务与技术工单新候选待预检

- 最终演示范围改为财务和技术工单。客服不进入新实验，但旧三场景实验中的客服失败、负匹配证据和全部成本保持历史可审计。
- 新模式 `finance_tickets_probe/formal` 已实现：预检固定4对，正式固定财务12项和技术工单12项；两臂使用双Key并行，读取工具全局仍串行。
- 无模型验证通过：Python 255项、前端35项、生产构建与 `git diff --check`。真实API预检与24对正式结果尚未产生，不能提前主张两场景收益。

## 2026-09-15 · 三场景扩大采用相对质量门槛

- 新冻结门槛允许在线 RSI 在12对中比“图执行 · 不学习”最多少通过1项；usage完整、维护正常和真实图执行率至少50%的要求不变。
- 严格 `qualityGate` 仍要求两臂全部通过，只有它允许同质量效率主张；相对质量门槛只允许继续采集stage2/3证据。所有失败、取消、token、请求和时长继续保留。
- 旧 `4cfbc988-fab0-4537-b7e2-e509c5cfd76a` 仍按其原12/12在线门槛保持 `gate_stopped`，不追溯改判。新规则只作用于新建并冻结的campaign。

## 2026-09-15 · 数字员工对比展示压缩

- 本轮只调整保存run的前端投影，没有调用模型、修改实验工件或改变当前Release Manifest。双轨执行区改为整页宽度，完整题面在两臂上方显示。
- 事件流水默认折叠；运行中改用右上角苹果通知式阶段卡，两臂独立按真实事件弹出、堆叠并在约3.1秒后淡出。卡片从保存事件显示当前表、16笔复核主体、10期/金额差异阈值、token、图节点和G版本等实例值；历史完成run恢复保持静默。前端35项测试、生产构建和浏览器布局检查通过，未调用模型或改写实验工件。

## 2026-09-15 · 分页式保存结果演示

- 当前金融12任务 candidate、实验工件和 Release Manifest 未改变，本轮没有模型调用。分析页改为五个逻辑标签，回放进度跨标签共享；浏览器已验证从业务结果切到记忆进化后仍按第1→第2项推进，并保持 FX02 的首次记忆复用高亮。
- 成本曲线、模型请求/可靠性、业务报告和逐任务轨迹不再同时堆叠在一个长页面。390px 页面无根级横向溢出，标签栏允许局部滚动。

## 2026-09-15 · 保存结果回放与首次跨任务记忆生效

- 当前金融12任务 candidate 的工件和 Release Manifest 均未改变；本轮没有模型调用。分析页可按保存任务顺序播放、暂停、重播并切换速度，所有累计指标、曲线、失败、修订和任务详情只显示当前回放前缀。
- FX01 是 G0/M0 的来源任务，状态保持“已配对”；FX02 首次选择并复用同一 G0/M0，因此蓝色高亮和“记忆已构建 · 首次复用”放在 FX02。G0/M0 创建继续不计作结构修订。

## 2026-09-15 · 金融12任务候选的展示结论

- 当前候选仍为 `d02f0ecd-8bb5-4359-94be-e7f7233df6a5`：在线 RSI 12/12，不学习图 Agent 11/12，24次运行usage完整。
- 对导师展示时，主结论是在线 RSI 在本组质量更高，同时token 2,119,742→936,718、模型请求165→58、串行时长1,081.771→742.943秒。上述效率变化来自同一固定12任务组并包含FX11失败与全部恢复开销。
- `candidate` 表示证据范围仍限于财务任务、没有独立 Judge 或人工全文审核；它不表示在线 RSI 质量差。由于两臂通过率不同，本组变化不称严格同质量消融，也不外推为跨业务长期收益。
- 前端已删除“质量受限/质量门槛未通过”主视觉，恢复本组离线累计效率曲线；实验、runtime、资产和协议仍可在技术审计与清单中追溯。
...
- 资产 `cross-domain-rsi-attribution-v1-48` 冻结三场景各16项、六子簇各8项；两臂同为图执行Agent，区别只在跨任务学习。
- smoke `80187bae-c980-4b59-8553-6d82ea054306` 两项双臂通过；它是启动检查，冷启动token 168,351→304,540，不进入收益结论。
- 旧 probe `3fe6c157-0a32-4268-b9bf-3c38e88b6381` 完成前7对：不学习6/7、在线RSI 7/7，实际图执行5/7，usage完整；绝对token 1,091,875→647,952，请求82→44，工具错误32→8。C10不学习臂失败仍保留。
- 新协议将 `qualityGate` 与 `expansionGate` 分开。完整12对中若在线RSI 12/12、两臂usage完整且真实图执行率≥50%，即使不学习臂有普通业务失败，也可扩大48对；但 `qualityGate=false` 时不能主张同质量效率，只能展示准确率与实际总开销差异。
- 新runtime将从空经验运行完整12对probe，普通业务失败后继续固定任务序列。48对formal仍未启动，当前发布继续使用金融12项candidate，直到新实验完整通过扩大门槛。完整历史见 [三场景预检](history/cross-domain-attribution-probe-results-2026-09-15.md)。

# 实验状态与可主张的结论

## 2026-09-14 · 十二任务候选分层交付

- 全量主结果不变：不学习11/12、在线RSI 12/12，`qualityGate=false`，FX11及全部成本保留。
- 运行前冻结的订单财务复核子簇 FX01–FX08 两臂均8/8：token 1,025,754→366,230（-64.30%），模型请求84→30（-64.29%），串行时间649.760→383.985秒（-40.90%），工具错误19→1。该分层满足同质量与usage门槛，但不把全量candidate升级为formal。
- 支付结构健康子簇 FX09–FX12 为3/4→4/4：绝对token 1,093,988→570,488、请求81→28、工具错误27→7、串行时间432.011→358.958秒。由于质量不等，百分比只作诊断；该子簇用于展示FX11失败、在线恢复、G2/M2生成及FX12实际使用。
- 事后排除FX11的11项不作为主指标；本轮不删除数据点、不重排任务、不追加付费模型运行。
...
- **资产与协议**：`finance-rsi-attribution-v5-12`；`finance-graph-rsi-error-recovery-probe-v1`
- **runtime**：`sha256:66f943479bc4b693e1b412e7af7d919f1609ebf21210eb1954c9a039a41d0cce`
- **顺序**：`FX01 → FX09 → FX10`；严格串行 `run=model=read=1`；27B统一规划/执行/组合，thinking关闭。

两臂各3/3通过且usage完整。不学习臂为609,447 token、47模型请求、67工具调用、12工具错误、369.756秒；在线RSI为324,130 token、23请求、60工具调用、3工具错误、184.904秒。FX01在线冷启动token增加46.1%；FX09、FX10分别观察到56.59%和78.92%的任务内token下降。这个分布说明累计差来自后续复用，不能把冷启动写成收益。

FX01创建G0/M0；FX09实际执行G0的13个节点，成功后产生实质覆盖扩展G1/M1；FX10实际执行G1/M1的18个节点并通过。G1新增支付记录计数、订单状态不同值统计和月份不同值统计，移除重复订单ID不同值统计。实际图使用2/3，达到50%探针门槛。运行后审计发现G1的 `payment_count` 节点把 `amount_cents` 求和；FX10虽执行该节点，但最终正确13条记录来自映射 `rowCount`/当前观察和模型判断。后续runtime已拒绝此类误导别名，旧工件不改写。请求分解为：读取选择11→1、计算选择28→16、混合决策1→0、报告组合3→3。

该实验只证明三个财务实例中的创建、复用、覆盖扩展和后续使用。协议明确排除 formal metrics；没有客服/技术工单、独立Judge或人工全文复核。本次没有先失败再恢复，不能称错误纠正。失败实验 `9ee28f75-59ba-42c2-8c56-739d4ad464c8` 与 `4aaf803b-d236-45d0-8d2c-827352ae47c9` 原样保留且不进入上述总量。

交付验证：230项Python测试、17项前端测试、TypeScript/Vite构建与 `git diff --check` 通过；浏览器实际检查数字员工、候选分析、FX09修订diff、FX10同run报告/清单/轨迹、52个历史上下文及窄屏布局。
...
> [V8 对账协议](history/workspace-workpack-v8-reconciliation-protocol-2026-09-12.md)。
...
> 边界见 [V12 结果](history/workspace-workpack-v12-results-2026-09-13.md)。
...
> 协议见 [V13 修复协议](history/workspace-workpack-v13-repair-protocol-2026-09-13.md)。

> V13 smoke `5dbcb2fc` 为 Baseline 2/3、RSI 3/3；财务一对多对账通过，但客服
> Baseline 把 `count + groupBy` 的总行数误作不同分组数。V14 已将此调用改为显式拒绝，
> V13 token 差仅诊断，详见 [V13 诊断](history/workspace-workpack-v13-results-2026-09-13.md)。
...
> [V15 Smoke](history/workspace-workpack-v15-smoke-results-2026-09-13.md)。

> V15 precheck `c64f1bca` 为 12/12 vs 12/12，Agent token 419,583→271,902
> （-35.197%）；第一轮形成 6 个 Workflow，第二轮 6 次实际 Fast 复用，0 维护错误。
> 它通过质量门槛；详情见
> [V15 Precheck](history/workspace-workpack-v15-precheck-results-2026-09-13.md)。
...
`online-e2e-train-v1` 的只读审计确认 25 次跨场景 TinyEdge 维护异常、两条能力/Plan 语义偏移轨迹以及无界重复报告失败。修复设计见 [online-rsi-repair-design-2026-09-10.md](history/online-rsi-repair-design-2026-09-10.md)。该实验保留为负向结果；共享报告恢复修复也会影响基线，不能把它与修复后的新实验连续累计。

两轮评测重复使用同一批验证任务，不是60个独立任务。测试集尚未用于最终性能测试。旧沙箱执行代码与专用运行记录已按用户要求删除；有日期的旧实验文档仍只代表历史结果，平台记录单独保留，不能混入此表。

来源：

- [Strong 基线结果](history/paired-evaluation-2026-09-10.md)
- [Plan 基线结果](history/plan-baseline-validation-2026-09-10.md)
- [在线复用验证](history/online-evolution-validation-2026-09-10.md)
- [Judge 分数与 reward](history/judge-reward-validation-2026-09-10.md)
- [Motif 初始训练证据](history/motif-filter-then-enrich-validation-2026-09-10.md)
- [G-Agent 局部组合初始验证](history/g-agent-local-composition-validation-2026-09-10.md)
- [成对评测协议](reference/paired-evaluation.md)、[裁判协议](reference/llm-judge.md)
...
## 2026-09-15 · 数据分析演示收口（非新增实验）

`#analysis` 已暂时移除“报告与轨迹”标签及对应的单任务详情请求；记忆进化页压缩为 12 项任务编号、单一关键状态与一行真实进化计数；进化与成本页首次进入为 0/12 空态，播放后再逐项累积。分析 API 强制学习归因两臂均为 `graph_rsi`，避免混入 Plan + ReAct；当前累计指标、失败、恢复与 artifact 未改写，也未运行新的模型调用。验证：前端测试 38/38、TypeScript/Vite 构建和 `git diff --check` 通过；浏览器确认主标签为四项，FX01–FX12 的首次复用、G/M 修订与新版本使用可在同屏辨认。
