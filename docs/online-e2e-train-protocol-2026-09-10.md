...
近期真实 run 约 20–35 秒/Agent；72 次 Agent 及 72 次 Judge 请求预计约 55–95 分钟，实际网络、限流和格式失败会计入记录。第一轮是同一正式实验的预检；协议正确时继续后五轮，不重跑首轮。系统性网络、限流或计量故障将暂停后续任务并保留已产生的 checkpoint。
