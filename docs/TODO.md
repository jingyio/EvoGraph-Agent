...
- 2026-09-14 `#home` 补齐岗位内 16 个冻结 train Workpack 题库与按需保存同题 pair：当前
  Workpack 才能显示 V17 的 Plan + ReAct / Graph RSI 指标、报告和原始轨迹，用户改资料或
  题目后不再保留同题标识。该展示不会启动额外 Agent；后续只需保持它与新实验工件的版本边界。
