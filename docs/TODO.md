...
状态：**运行前修复完成，待执行**。V3 9B 预检首对已失败并保留为历史；不得续跑或替换。

- [x] 复核 F01 失败收据，确认是公开 ID 契约歧义而非私有答案泄漏、数据缺失或模型收益。
- [x] 创建 `trajectory-review-v3-r1`：相同记录、顺序、切分和评分，新增公开 `selectedIdField`。
- [x] 两臂共享报告 ID 契约、完整观察后的报告恢复限制和 live 工具调用要求。
- [x] 通过 V3/V3-r1 资产、Workspace、TaskRunner、Release、模型协议相关 71 项 Python 测试。
- [ ] 从新空 RSI 经验库启动 12 对 `qwen/qwen3.5-9b` 严格串行 V3-r1 预检；任何首对失败即停止。
- [ ] 仅在 12 对均同质量、usage 完整且维护门槛通过后，冻结并启动 48 对正式 train 对照。
- [ ] 依据实际 V3-r1 工件更新当前证据、教师交付和录制脚本；未通过前只展示候选审阅与失败诊断。
...
- **已完成** `trajectory-review-v2`：三场景×两个公开业务问题×八个train，共48项；另留6 validation/6 test。根目录 `test/` 使用同一问题定义，旧V17/V1/V2不改写。
- 题面多写业务目标、约束和交付，少指定执行流程；不提供工具名/调用顺序，必要业务口径保留。
- **等待用户审批**：先在当前证据页审阅六个契约、输入字段、48个train题面与自然cohort限制；批准后只运行固定9对严格串行预检。质量、usage与维护门槛全部通过且预算条件满足后，才允许48项full。 工具契约审阅已冻结：两臂共享20个工作区工具，实验报告提交前置要求精确metrics与完整groups。
...
- 当前Release Manifest指向V3待运行候选，计划48、已运行0；旧V2的24train资产和4对失败预检只在历史区。
