...
`filter_then_enrich` 是一项局部 Motif 修改：正常训练 Plan 若声明可验证的列表字段条件，G0 或后继图保存条件、来源节点和训练运行 ID。它不代表仅凭一次筛选就产生了 G1；结构未变化只累计证据。`plan_react_reuse` 是显式执行对照，复用同一保存 Plan 但不执行图、不写进化证据，用于拆开规划缓存成本与编译执行成本。
