...
import { cancelRun, restoreRuns, runs, startRun, toolsFor } from './service.js';
...
app.get('/api/snapshot', (_req, res) => res.json(seedWorld()));
...
app.get('/api/runs', (_req, res) => res.json([...runs.values()].sort((a, b) => b.startedAt.localeCompare(a.startedAt)).slice(0, 30).map(run => ({ id: run.id, request: run.request, status: run.status, startedAt: run.startedAt, metrics: run.metrics }))));
app.post('/api/runs', async (req, res) => { const run = await startRun(req.body); res.status(202).json({ id: run.id }); });
app.get('/api/runs/:id', (req, res) => { const run = runs.get(req.params.id); if (!run) { res.status(404).json({ error: 'Run not found' }); return; } res.json(run); });
