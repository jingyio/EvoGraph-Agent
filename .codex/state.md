...
- 数字员工页已改为一次启动同题同附件的两个真实 run：传统 `plan_react` 与在线 `graph_rsi`。两臂创建后同时显示，后端保持严格串行模型槽，前端每800ms轮询真实事件、用量、错误与报告；不生成模拟进度。
- 长题面输入框按内容自动增高，桌面双栏、窄屏上下堆叠。数据分析页已按筛选范围隔离曲线、G/M修订与后续使用，并用橙色/紫色高亮有来源和实质diff的真实G/M修订。
- 跨场景实验改为同一 campaign 的12→24→48：创建时冻结全部48项和三阶段顺序；stage1 12对通过扩大门槛后自动执行stage2 12对。24对完成后可显式恢复同一experiment、runtime与在线经验库，只追加stage3剩余24对，不重跑前24对。
- 源任务资产仍为48 train、6 validation、6 test；今天的campaign只学习train，6个test题面已同步V3-r3自然业务风格并保持隔离。
