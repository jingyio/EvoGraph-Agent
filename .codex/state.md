...
- 首屏只保留测试组与候选状态、12/12对11/12、业务质量、token/模型请求效率以及G/M修订后使用。USD估算、完整题面、工具调用、冻结子簇、逐任务账本和维护诊断默认折叠。
- 五个演示标签共享同一保存回放前缀；普通连续复用弱化，首次复用和真实G/M修订链保持醒目标记。桌面和390px浏览器闭环通过，移动端根宽度为390px且无横向溢出。
- 验证：前端35/35、`npm run build`、`git diff --check`通过。实现仅涉及`src/DataAnalysis.tsx`、`src/data-analysis.css`及本轮状态文档。
