...
- binder 现在只忽略未被选中节点及其依赖闭包使用的多余槽，并在 `trajectoryMatchNormalization` / `trajectory_match_normalized` 保留审计。选中节点缺槽、任何重复槽、当前题面无法逐字验证、类型或单位不兼容仍失败关闭；不会增加节点、猜值或补业务答案。
- 按用户要求取消新的不学习臂，不再完成第二组三臂验证。在线 RSI 单臂 run `1c904b6b-9f90-46f6-b3f8-b0a522ab6998` 真实执行通过：3次模型请求、49,496输入/1,291输出token、25次工具调用、0工具错误、33.108秒，实际使用冻结 G2/M2 `1fd5c91d-64f4-4e7f-8193-a9b41d64b158` 并执行23个图节点。三次请求分别为匹配、补一个未覆盖比较、生成报告；业务结果与前一成功组一致，结构化状态为 `user_review_required` 且 issues为空。
- 这次单臂运行证明“先前无收益由匹配回退造成，修复后图回放恢复”，但不是新的成对时延/质量实验。可与同题旧不学习臂作诊断参照，不能替代冻结成对正式结论。旧失败、取消臂和全部成本均保留。
- 验证：264项Python、39项前端测试、TypeScript/Vite生产构建与`git diff --check`通过。

## 2026-09-15 恢复正常 Agent 设计的并行诊断（历史）

- 先对照金融12任务正常实验、冻结源码与最新失败run，确认基础 system prompt 和 `STRONG_REACT_GUIDANCE` 未发生实质变化。退化来自普通工作区缺少公开证据范围、模型重复序列化18–58条长 evidence ID、冻结 G2/M2 因未审核状态与 contract hash 变化无法召回，以及 `b9ad8fd` 的演示专用首份报告终止/提前预算边界。
- 新三臂恢复共享正常协议：三臂获得相同 `granular-compute-v1` 工具面和当前附件范围；模型继续负责 metrics、业务ID、原因组与正文，运行时只从本次已观察资料和成功计算收据绑定顶层/分组证据，不读取 gold。报告失败重新进入共享有界恢复。在线RSI仅在显式 `frozen_finance_release` 下允许读取清单绑定的 probation 版本，并逐节点重验当前工具、effect、依赖和输出路径；实际参数仍由当前 schema 校验。
- 三臂恢复双Key并行：A传统Plan+ReAct与B图执行不学习使用主Key，C在线RSI使用Secondary Key，调度为`run/model/read=3/3/3`。历史串行run与失败不改写。
- 唯一真实API验证 comparison `9c354b04-6373-4333-80d2-8967b8d50dc1` 使用同一题目与58行附件。A：15请求、183,093输入/2,864输出token、128.278秒；B：12请求、172,475输入/2,525输出token、95.890秒；C：3请求、50,876输入/1,135输出token、31.441秒。三臂独立得到16订单、两侧合计496,883分、差异0、10期及以上3个且ID一致、缺失0。
- C真实命中并执行冻结 `G2/M2` 版本 `1fd5c91d-64f4-4e7f-8193-a9b41d64b158` 的23个节点，阈值按当前任务绑定为10，随后只用1次模型计算补齐未覆盖义务并生成报告；未复制旧业务结果。三臂报告请求输出分别为791/808/602 token，证明输出膨胀在确定性证据绑定后消失，暂不增加 completion cap。
- 验证：Python 264项、前端39项、生产构建和`git diff --check`通过。当前Release/Analysis Manifest仍指向原金融12任务candidate；本次自由工作区run为`user_review_required`，不进入当前发布指标。

## 2026-09-15 严格串行三臂与失效任务静默恢复（最新）

- `#home` 新建三臂对照固定按 A 传统 `Plan + ReAct` → B 图执行不学习 → C 图执行在线 RSI 依次运行，三臂共用主 Key，工作区调度冻结为 `run/model/read=1/1/1`；DTO 标记 `executionPolicy=strict_serial_three_arm`。历史并行三臂继续保留原策略标记，不改写旧 run。
- 历史 comparison `e68639ea-b793-4503-9f30-fa429d01a3ad` 的 A/B “受限停止”不是 API 限流：A 在完成主要计算后继续读取证据，未给终态报告保留完整模型时限；B 在成功计算后以相同参数重复调用20次，耗尽请求预算。C 正常完成。
- 运行时现会在当前附件已有成功确定性计算时为终态报告预留完整时限；相同成功计算连续两次被重复调用后转入仅允许报告工具的有界完成阶段。严格串行演示在完整 `workspace_reconcile_keyed_sums` 成功后立即进入500字以内的精简报告，首次报告保存即结束该臂；校验失败保留失败状态和报告，但不再追加一次模型恢复。正式离线实验仍保留原恢复协议。
- 删除附件后，历史任务通过 `sourceStatus` 标记失效。页面静默恢复遇到失效任务时清除本地 comparison/readyTask 引用，不再弹出红色“任务引用的资料已被移除”提示；主动操作产生的其他真实错误仍正常显示。后端历史 run、报告和轨迹不删除。
- 验证：Python 262项、前端39项、生产构建、`git diff --check` 及三个已打开 `#home` 页面检查通过；页面均无旧红字并显示 A→B→C 严格串行。本轮没有启动新的付费模型实验。

## 2026-09-15 实测三臂演示收口（最新）

- `#home` 的三臂为A传统 `Plan + ReAct`、B空经验图执行、C只读金融经验图执行；A→B说明图运行时差异，B→C才是跨任务学习贡献。
- C臂不再使用代码硬编码实验ID，而是从 `releases/analysis-manifest.json.defaultDatasetId` 解析；后端校验attribution类型、工件摘要、runtime、资产、协议、纯财务范围和经验文件后才允许启动。当前清单仍指向金融12任务candidate的3个版本。
- 页面首屏只保留三张一句话方法卡、两条归因和完整题面。协议、双Key和知识库身份默认折叠；未启动时隐藏三组空指标、空报告与重复等待面板，启动后显示真实阶段、指标、报告和审计。
- 工作区调度为`run=3/model=3/read=3`，测试确认三臂首个模型请求可同时进行。旧两臂与旧Plan/Graph记录继续恢复。
- 验证：Python259项、前端38项、生产构建、diff检查和1280px浏览器无溢出检查通过。费用确认未勾选，本轮没有新增付费模型run或改写历史实验工件。

## 2026-09-15 实测三臂与进度修正

- `#home` 现在同时展示传统 `Plan + ReAct`、图执行不学习、在线RSI三条真实run。传统参考用于回答Baseline没有被故意削弱；不学习图Agent与在线RSI之间才是学习消融。
- 三臂均使用27B、同题、同附件、同工具和预算。主Key承载Plan+ReAct与不学习臂，Secondary Key承载在线RSI。实测工作区调度为`run=3/model=3/read=3`，消除旧`read=1`导致的工具阶段排队；正式归因实验协议不变。
- B仍为空经验且禁止跨任务读写；C只读加载金融12任务candidate `d02f0ecd-8bb5-4359-94be-e7f7233df6a5`的3个版本，不回写演示任务。
- 右上弹窗和执行正文已同步使用三臂身份；页面新增真实阶段对照和当前事件实际持续时间，不生成虚假进度。旧两臂与旧Plan/Graph历史comparison继续兼容。
- 本轮未启动新的付费模型运行、未切换Manifest、未改写实验工件。浏览器已准备三臂费用确认前状态。

## 2026-09-15 实测对比与金融冻结知识库（最新）

- 主导航将 `#home` 命名为“实测对比”，产品总名称仍为“数字员工”；实测页固定财务运营，不再展示客服或技术工单切换。
- 实测双臂已与当前金融归因协议对齐：A/B 都使用 `graph_rsi` 图执行运行时。A 使用独立空经验，每次重新规划/编译且不读写跨任务经验；B 只读加载金融12任务 candidate `d02f0ecd-8bb5-4359-94be-e7f7233df6a5` 的 `online_rsi/experience.json`，当前加载3个保存版本。
- 两臂仍使用 `qwen/qwen3.5-27b`、主Key/Secondary Key并行和 `run=2/model=2/read=1`。实测任务不会回写冻结知识库，因此不会污染当前证据；run DTO保存 arm、知识库来源、版本数和只读状态。
- `#analysis` 新增醒目的A/B总览：不学习臂为灰蓝，在线RSI为绿色；通过数、token、模型请求、串行时长及正负变化均从同一金融Release Manifest计算。未运行新的付费模型实验。

## 2026-09-15 数据分析页展示收口（最新前端状态）

- `#analysis` 当前仍只读金融12任务 candidate `d02f0ecd-8bb5-4359-94be-e7f7233df6a5`；本轮未切换Release Manifest、未修改工件、未运行付费模型。
- 首屏只保留测试组与候选状态、12/12对11/12、业务质量、token/模型请求效率以及G/M修订后使用。累计效率三图并列置于成本页，保存结果控制栏在桌面端置于右侧。
- 报告页删除截断摘要、冻结子簇、逐任务账本、维护诊断、数据限制长列表和模型请求内部分类；两臂完整报告改为按需展开，业务报告、结构化清单、真实轨迹、工具调用与真实G/M diff仍可审计。
- 五个演示标签共享同一保存回放前缀；普通连续复用弱化，首次复用和真实G/M修订链保持醒目标记。桌面和390px浏览器闭环通过，移动端根宽度为390px且无横向溢出。
- 验证：前端35/35、`npm run build`、`git diff --check`通过。本轮未改动实验artifact、后端数据或Release Manifest。

## 2026-09-15 工单独立扩大（最新）

- 用户决定财务12项不重复运行，沿用候选实验 `d02f0ecd-8bb5-4359-94be-e7f7233df6a5`；新付费实验仅运行技术工单 `T01–T12`。
- `tickets_probe` 固定 `T01,T02`，`tickets_formal` 固定 `T01–T12`；主 Key 与 Secondary Key 按 pair 同时运行不学习/在线RSI两臂，模型为 `qwen/qwen3.5-27b`，thinking关闭，全局 `run=2/model=2/read=1`。
- 正式工单实验必须由同 runtime、同资产且两臂全通过的预检解锁；usage、维护和至少50%真实图执行是硬门槛。客服和此前混合运行完整保留历史，不进入最终主展示。
- 最终页面只提供财务、技术工单两个独立证据上下文；各自计算指标和曲线，禁止跨实验合计。
...
- 数据分析页新增只读“保存结果回放”：播放、暂停、重播、三档速度和最终结果。回放按真实任务顺序逐项投影 KPI、累计曲线、可靠性、任务详情、G/M 修订及后续使用，只消费已有 artifact，不重新调用模型。模型请求内部分类不进入主展示。
...
`#analysis` 当前只保留“业务结果 / 记忆进化 / 成本曲线 / 请求与可靠性”四个演示标签；“报告与轨迹”及其单任务详情请求已暂时移除，原始报告和轨迹 artifact 未删除。记忆进化页以 FX01–FX12 编号和单一状态展示首次复用、结构修订及新版本使用，底部仅汇总真实图执行、G/M修订和后续使用；首次进入进化或成本页显示 0/12 空态，播放后才逐项揭示。分析 API 同时强制学习归因两臂均为 `graph_rsi`，防止混入 Plan + ReAct。未调用模型、未改写实验结果。验证：前端 38/38、构建、diff check 和桌面浏览器通过。
